...
function parse$6(command2, args, options2) {
...
function spawn(command2, args, options2) {
  const parsed = parse$5(command2, args, options2);
...
var stringify$4 = (ast, options2 = {}) => {
  const stringify2 = (node, parent = {}) => {
    const invalidBlock = options2.escapeInvalid && utils$8.isInvalidBrace(parent);
...
const expand$1 = (ast, options2 = {}) => {
  ...
  const walk = (node, parent = {}) => {
    ...
    if (node.nodes && node.ranges > 0) {
      ...
      if (utils$6.exceedsLimit(...args, options2.step, rangeLimit)) {
...
const parse$2 = (input, options2) => {
  ...
  const win32 = utils$3.isWindows(options2);
  ...
  if (opts.fastpaths !== false && !/(^[*!]|[/()[\]{}"])/.test(input)) {
    ...
    state.output = utils$3.wrapOutput(output, state, options2);
    return state;
  }
  while (!eos()) {
    value = advance();
...
const picomatch$2 = (glob, options2, returnState = false) => {
  ...
  const opts = options2 || {};
  const posix = utils$2.isWindows(options2);
...
picomatch$2.matchBase = (input, glob, options2, posix = utils$2.isWindows(options2)) => {
...
picomatch$2.parse = (pattern, options2) => {
  ...
  return parse$1(pattern, { ...options2, fastpaths: false });
};
...
micromatch.capture = (glob, input, options2) => {
  let posix = utils$1.isWindows(options2);
...
const traverseEntity = async (visitor2, options2, entity) => {
  const { path: path2 = { raw: null, attribute: null }, schema: schema2, getModel } = options2;
...
const visitor$4 = ({ schema: schema2, key, value }, { set: set2 }) => {
  if (key === "" && value === "*") {
    ...
    const newPopulateQuery = Object.entries(attributes2).filter(
...
const traverseFactory = () => {
  ...
  const traverse = async (visitor2, options2, data) => {
    ...
    for (const { predicate, handler } of state.interceptors) {
      if (predicate(data)) {
        return handler(visitor2, options2, data, { recurse: traverse });
      }
    }
  ...
  async (visitor2, options2, filters2, { recurse }) => {
    return Promise.all(
      filters2.map((filter, i) => {
        const newPath = options2.path ? { ...options2.path, raw: `${options2.path.raw}[${i}]` } : options2.path;
        return recurse(visitor2, { ...options2, path: newPath }, filter);
      })
...
const sort = traverseFactory().intercept(
  ...
  async (visitor2, options2, sort2, { recurse }) => {
    return Promise.all(
      sort2.split(",").map(fp.trim).map((nestedSort) => recurse(visitor2, options2, nestedSort))
  ...
  isStringArray$2,
  async (visitor2, options2, sort2, { recurse }) => {
    return Promise.all(sort2.map((nestedSort) => recurse(visitor2, options2, nestedSort))).then(
...
const populate = traverseFactory().intercept(isPopulateString, async (visitor2, options2, populate2, { recurse }) => {
  ...
  const paths = await Promise.all(
    populate2.map((subClause) => recurse(visitor2, options2, subClause))
  );
  ...
  async ({ key, visitor: visitor2, path: path2, value, getModel, parent }, { set: set2, recurse }) => {
    ...
    for (const [uid2, subPopulate] of Object.entries(value)) {
      ...
      const newPath = { ...path2, raw: `${path2.raw}[${uid2}]` };
      newOn[uid2] = await recurse(
        visitor2,
...
const fields = traverseFactory().intercept(isStringArray, async (visitor2, options2, fields2, { recurse }) => {
  ...
  (visitor2, options2, fields2, { recurse }) => {
...
const product$2 = factories.createCoreController(
  ...
  return async (ctx, next) => {
    ...
    try {
      ...
      if (ts && hash && dataID && xRequestId) {
        ...
        console.log(hash);
        if (sha === hash) {
          strapi2.log.info("Webhook Auth Success");
          return next();
        } else {
          strapi2.log.info("Webhook Auth Failed");
          return ctx.serviceUnavailable("Service Unavailable");
        }
      } else {
        strapi2.log.warn("Missing required values:");
        if (!ts) strapi2.log.warn("ts is missing");
        if (!hash) strapi2.log.warn("hash is missing");
        if (!dataID) strapi2.log.warn("dataID is missing");
        if (!xRequestId) strapi2.log.warn("xRequestId is missing");
...
const productPriceSummary = (product2) => {
...
const productsPricesSummary = (products) => {
  const pricingInfo = products.map((product2) => {
    const { fullPrice, fullPriceDiscount, totalDiscounted: totalDiscounted2, finalPrice } = productPriceSummary(product2);
    return {
      fullPrice,
      fullPriceDiscount,
      totalDiscounted: totalDiscounted2,
      finalPrice,
      ...product2
    };
  });
  const totalFullPrice = pricingInfo.reduce(
    (acum, product2) => acum + product2.fullPrice,
    0
  );
  const totalFullPriceDiscount = pricingInfo.reduce(
    (acum, product2) => acum + product2.fullPriceDiscount,
    0
  );
  const totalDiscounted = pricingInfo.reduce(
    (acum, product2) => acum + product2.totalDiscounted,
    0
  );
...
const middlewares = { loadConfig, verifySign, populating };
const policies = {};
...
const product = factories.createCoreService("plugin::strapi-mercadopago.product", ({ strapi: strapi2 }) => ({
  getProducts: async (items) => {
    ...
    const sku = items.map(({ sku: sku2 = "" }) => ({ sku: sku2 }));
